        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-striped display compact">
                    <tbody>
                        <tr><td><strong>id:</strong></td><td>{{ $cordon->id }}</td></tr>
                        <tr><td><strong>Nombre:</strong> </td><td>{{$cordon->nombre}}</td></tr>
                        <tr><td><strong>Metros:</strong></td><td>{{$cordon->metros}}</td></tr>
                        <tr><td><strong>Coordenadas:<strong></td><td>{{ $cordon->coordenadas }}</td></tr>
                        <tr><td><strong>Typo:<strong></td><td>{{ $cordon->typo }}</td></tr>
                        <tr><td><strong>Ancho:<strong></td><td>{{ $cordon->ancho }}</td></tr>
                        <tr><td><strong>Estado:</strong></td><td><div class="btn btn-{{$cordon->status->color}} btn-sm">{{$cordon->status->name}}</div></td></tr>
                        <tr><td><strong>Tiempo:</strong></td><td>{{$cordon->created_at->diffForHumans()}}</td></tr>
                    </tbody>
                   
                </table>
                
            </div>
            <td><a href="{{route('admin.cloaca.edit',$cordon)}}" class="btn btn-secondary">Editar</a></td>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card">
...
<script>
    let inputArray = [{{ $cordon->coordenadas }}];
...
console.log(ubics);
   ...
   L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    ...
    var polyline = L.polyline(ubic, {color: 'red'}).addTo(map);
